...
func (a *actuator) Delete(ctx context.Context, be *extensionsv1alpha1.BackupEntry) error {
	factory, err := azureclient.NewAzureClientFactory(ctx, a.client, be.Spec.SecretRef)
	if err != nil {
		return err
	}

	return factory.Storage().DeleteObjectsWithPrefix(ctx, be.Spec.BucketName, fmt.Sprintf("%s/", be.Name))
}
