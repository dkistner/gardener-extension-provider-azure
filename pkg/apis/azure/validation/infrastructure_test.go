			...
			Context("User provided public IP", func() {
				...
				It("should fail as resource is in a different zone as the NatGateway", func() {
					...
					Expect(errorList).To(ConsistOfFields(Fields{
						...
						"Detail": Equal("Public IP can't be used as it is not the same zone as the NatGateway (zone 1)"),
					}))
				})
				...
				It("should fail as name is empty", func() {
					...
					Expect(errorList).To(ConsistOfFields(Fields{
						...
						"Detail": Equal("Empty name for NatGateway public IP is invalid"),
					}))
				})
				...
				It("should fail as resource group is empty", func() {
					...
					Expect(errorList).To(ConsistOfFields(Fields{
						...
						"Detail": Equal("Empty resourceGroup for NatGateway public IP is invalid"),
					}))
				})
			})
