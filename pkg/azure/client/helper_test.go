...
var _ = Describe("Helper", func() {
	DescribeTable("#AzureAPIErrorNotFound",
		func(isDetailedError, hasResponse bool, statusCode int, expectIsNotFoundError bool) {
			var err = errors.New("error")
			if !isDetailedError {
				Expect(AzureAPIErrorNotFound(err)).To(Equal(expectIsNotFoundError))
				return
			}
			...
			Expect(AzureAPIErrorNotFound(detailedError)).To(Equal(expectIsNotFoundError))
		},
		Entry("should return false as error is no detailed azure error", false, false, 999, false),
